...
- **DataScope 写路径行级校验：四域 Update/Delete 补 owner/role 守卫（2026-09-11）** — 架构缺陷清单最后一个挂账项。此前读路径五域均已注入 DataScope（普通角色仅见本人创建/受理单据），但写路径（Update/Delete/BatchDelete）仅校验 tenantID+存在性，普通角色可通过猜 ID 直接改/删**他人**单据（读不到但写得到，权限不对称）。修复：①`handlers/common/datascope` 新增共享判定 `CanWriteResource(actorID, actorRole, ownerID, assigneeID)`——原则**写权限 ⊆ 读权限**，管理角色（IsDataScopeAllRole）全租户可写，其余角色仅 owner（ticket.RequesterID/change.CreatedBy/problem.CreatedBy/incident.ReporterID）或当前受理人可写；actorID<=0、未知角色一律安全收窄拒绝；②ticket/change/problem/incident 四域 service 层 Update/Delete/BatchDelete（含 ticket 子任务、problem Investigate/RootCause/Solution/Close 转调链）统一签名注入 actorID/actorRole 并在加载实体后校验，BatchDelete 逐单校验（全有或全无，防批量接口越权）；③handler 层从鉴权中间件注入 `user_id`/`role`；④错误映射收口：行级拒绝返回 403 AppError，ticket 域原 isForbiddenErr 分支误用 FailWithErr 兜底 500 已改 RespondError 语义分流。测试：datascope 包 4 表驱动用例（管理角色/owner/受理人/安全收窄）+ problem 域真实 ent 链路越权 403 用例 + ticket handler 级越权 403 端到端用例。已知边界：incident 生命周期操作（assign/acknowledge/resolve/close，走生产层状态机）与 change 治理字段守卫外的审批动作暂未纳入行级守卫，待下一批统一评估；service_request 此前已有 isServiceRequestAdmin 守卫未动。
