...
var ResourceActionMap = map[string]map[string]Permission{
	"GET": {
		...
		// 批次 3（2026-09-17）：无条目 → L3 按 URL 推断复数资源名，与单数词表永不匹配
		// （实测 admin 持有对应 read 码仍被预检 403），显式登记消除推断失配
		...
		// 批次 3（2026-09-17）：预检以路由声明为权威对齐（TestRoutePrecheckAlignment 强制）
	...
	"POST": {
		...
		"/api/v1/ticket-categories":     {Resource: "ticket_category", Action: "create"}, // 批次3：对齐为路由声明
		...
		"/api/v1/ticket-templates/*":    {Resource: "ticket_template", Action: "write"},
		"/api/v1/ticket-tags":           {Resource: "ticket_tag", Action: "create"}, // 批次3：对齐为路由声明
		...
		"/api/v1/changes/*":             {Resource: "change", Action: "write"},
		// Release Management permissions（与路由层 RequirePermission("release", ...) 对齐；
		// 补充前 releases 端点在 DBOnly 模式下对非通配角色不可达——URL 推导取复数段
		// "releases" 而词表是单数 "release"，永不匹配）
		...
		// 批次 3：POST 承载只读语义（search/analyze/monitor/analytics/export），
		// 路由层声明 read，预检同步 read——角色边界归 RBAC，
		// AI 配额与巡检触发频率归限流/配额机制，不由 RBAC 承担
		...
		"/api/v1/tickets/analytics/deep":    {Resource: "report", Action: "read"},   // 批次3：对齐为路由声明
		"/api/v1/tickets/prediction/trend":  {Resource: "report", Action: "read"},   // 批次3：对齐为路由声明
		"/api/v1/tickets/prediction/export": {Resource: "report", Action: "read"},   // 批次3：对齐为路由声明
		"/api/v1/bpmn/ai/preview":           {Resource: "workflow", Action: "read"}, // 批次3：对齐为路由声明
		// 批次 3：写路径无条目 → 推断失配，显式登记（与路由层声明对齐）
	...
	"PUT": {
		...
		"/api/v1/ticket-categories/*":  {Resource: "ticket_category", Action: "update"}, // 批次3：对齐为路由声明
		"/api/v1/ticket-templates/*":   {Resource: "ticket_template", Action: "write"},
		"/api/v1/ticket-tags/*":        {Resource: "ticket_tag", Action: "update"}, // 批次3：对齐为路由声明
		"/api/v1/users/*":              {Resource: "user", Action: "write"},
		...
		"/api/v1/tickets/views/*":            {Resource: "ticket", Action: "write"}, // 视图 CRUD 按工单写权（批次2收敛）
	...
	"DELETE": {
		...
		// 批次 3：写路径无条目 → 推断失配，显式登记
		"/api/v1/tenants/*":      {Resource: "tenant", Action: "write"},
		"/api/v1/menus/*":        {Resource: "system_config", Action: "write"},
		"/api/v1/vendors/*":      {Resource: "vendor", Action: "delete"}, // 批次3：对齐为路由声明
		"/api/v1/surveys/*":      {Resource: "survey", Action: "write"},
		"/api/v1/connectors/*":   {Resource: "connector", Action: "write"},
		"/api/v1/applications/*": {Resource: "application", Action: "write"},
		"/api/v1/cloud/*":        {Resource: "cmdb", Action: "write"},
