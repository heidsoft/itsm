...
func (aq *ApplicationQuery) QueryMicroservices() *MicroserviceQuery {
	query := (&MicroserviceClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ApplicationQuery) Clone() *ApplicationQuery {
	...
	return &ApplicationQuery{
		...
		withMicroservices: aq.withMicroservices.Clone(),
...
func (aq *ApplicationQuery) WithMicroservices(opts ...func(*MicroserviceQuery)) *ApplicationQuery {
	query := (&MicroserviceClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withMicroservices = query
	return aq
}
...
func (aq *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := aq.withMicroservices; query != nil {
		if err := aq.loadMicroservices(ctx, query, nodes,
...
func (aq *ApplicationQuery) loadMicroservices(ctx context.Context, query *MicroserviceQuery, nodes []*Application, init func(*Application), assign func(*Application, *Microservice)) error {
