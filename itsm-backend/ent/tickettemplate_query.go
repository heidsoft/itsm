...
func (_q *TicketTemplateQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*TicketTemplate, init func(*TicketTemplate), assign func(*TicketTemplate, *Ticket)) error {
	...
	for _, n := range neighbors {
		fk := n.ticket_template_tickets
		if fk == nil {
			return fmt.Errorf(`foreign-key "ticket_template_tickets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "ticket_template_tickets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
