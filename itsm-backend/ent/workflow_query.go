...
func (wq *WorkflowQuery) QueryTenant() *TenantQuery {
...
func (wq *WorkflowQuery) QueryFlowInstances() *FlowInstanceQuery {
	query := (&FlowInstanceClient{config: wq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := wq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (wq *WorkflowQuery) Clone() *WorkflowQuery {
	...
	return &WorkflowQuery{
		...
		withFlowInstances: wq.withFlowInstances.Clone(),
...
func (wq *WorkflowQuery) WithTenant(opts ...func(*TenantQuery)) *WorkflowQuery {
	query := (&TenantClient{config: wq.config}).Query()
...
func (wq *WorkflowQuery) WithFlowInstances(opts ...func(*FlowInstanceQuery)) *WorkflowQuery {
	query := (&FlowInstanceClient{config: wq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	wq.withFlowInstances = query
	return wq
}
...
func (wq *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := wq.withTenant; query != nil {
		if err := wq.loadTenant(ctx, query, nodes, nil,
			func(n *Workflow, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	if query := wq.withFlowInstances; query != nil {
		if err := wq.loadFlowInstances(ctx, query, nodes,
			func(n *Workflow) { n.Edges.FlowInstances = []*FlowInstance{} },
			func(n *Workflow, e *FlowInstance) { n.Edges.FlowInstances = append(n.Edges.FlowInstances, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (wq *WorkflowQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Tenant)) error {
...
func (wq *WorkflowQuery) loadFlowInstances(ctx context.Context, query *FlowInstanceQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *FlowInstance)) error {
	...
	query.Where(predicate.FlowInstance(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.FlowInstancesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.workflow_flow_instances
		if fk == nil {
			return fmt.Errorf(`foreign-key "workflow_flow_instances" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "workflow_flow_instances" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (wq *WorkflowQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := wq.ctx.Fields; len(fields) > 0 {
		...
		if wq.withTenant != nil {
			_spec.Node.AddColumnOnce(workflow.FieldTenantID)
		}
	}
