...
func (sq *SubscriptionQuery) QueryTenant() *TenantQuery {
...
func (sq *SubscriptionQuery) WithTenant(opts ...func(*TenantQuery)) *SubscriptionQuery {
	query := (&TenantClient{config: sq.config}).Query()
...
func (sq *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if query := sq.withTenant; query != nil {
		if err := sq.loadTenant(ctx, query, nodes, nil,
			func(n *Subscription, e *Tenant) { n.Edges.Tenant = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubscriptionQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Tenant)) error {
...
func (sq *SubscriptionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withTenant != nil {
			_spec.Node.AddColumnOnce(subscription.FieldTenantID)
		}
	}
