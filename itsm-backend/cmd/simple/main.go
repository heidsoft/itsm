...
func main() {
	...
	// CORS — 仅允许配置的来源，不再使用通配符
	allowedOrigin := os.Getenv("CORS_ALLOWED_ORIGIN")
	if allowedOrigin == "" {
		allowedOrigin = "http://localhost:3000" // 默认仅允许本地前端
	}
	...
	// API Key 认证中间件（开发工具必需基本安全）
	apiKey := os.Getenv("API_KEY")
	if apiKey == "" {
		fmt.Println("⚠️  警告: API_KEY 环境变量未设置，API 无认证保护！")
		fmt.Println("⚠️  生产环境请设置 API_KEY 环境变量")
	}
	r.Use(func(c *gin.Context) {
		if apiKey == "" {
			c.Next()
