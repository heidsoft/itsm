...
func (_u *ChangeUpdate) check() error {
	if v, ok := _u.mutation.Title(); ok {
		if err := change.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`migrations: validator failed for field "Change.title": %w`, err)}
		}
	}
	if v, ok := _u.mutation.CreatedBy(); ok {
		if err := change.CreatedByValidator(v); err != nil {
			return &ValidationError{Name: "created_by", err: fmt.Errorf(`migrations: validator failed for field "Change.created_by": %w`, err)}
		}
	}
	if v, ok := _u.mutation.TenantID(); ok {
		if err := change.TenantIDValidator(v); err != nil {
			return &ValidationError{Name: "tenant_id", err: fmt.Errorf(`migrations: validator failed for field "Change.tenant_id": %w`, err)}
		}
	}
	return nil
}
...
func (_u *ChangeUpdateOne) sqlSave(ctx context.Context) (_node *Change, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "Change.id" for update`)}
	}
