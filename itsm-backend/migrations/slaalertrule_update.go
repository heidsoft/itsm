...
func (_u *SLAAlertRuleUpdate) check() error {
	...
	if _u.mutation.SLADefinitionCleared() && len(_u.mutation.SLADefinitionIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "SLAAlertRule.sla_definition"`)
	}
	return nil
}
...
func (_u *SLAAlertRuleUpdateOne) sqlSave(ctx context.Context) (_node *SLAAlertRule, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "SLAAlertRule.id" for update`)}
	}
