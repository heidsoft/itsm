...
func (_u *IncidentUpdate) check() error {
	if v, ok := _u.mutation.Title(); ok {
		if err := incident.TitleValidator(v); err != nil {
			return &ValidationError{Name: "title", err: fmt.Errorf(`migrations: validator failed for field "Incident.title": %w`, err)}
		}
	}
	if v, ok := _u.mutation.IncidentNumber(); ok {
		if err := incident.IncidentNumberValidator(v); err != nil {
			return &ValidationError{Name: "incident_number", err: fmt.Errorf(`migrations: validator failed for field "Incident.incident_number": %w`, err)}
		}
	}
	if v, ok := _u.mutation.ReporterID(); ok {
		if err := incident.ReporterIDValidator(v); err != nil {
			return &ValidationError{Name: "reporter_id", err: fmt.Errorf(`migrations: validator failed for field "Incident.reporter_id": %w`, err)}
		}
	}
	if v, ok := _u.mutation.TenantID(); ok {
		if err := incident.TenantIDValidator(v); err != nil {
			return &ValidationError{Name: "tenant_id", err: fmt.Errorf(`migrations: validator failed for field "Incident.tenant_id": %w`, err)}
		}
	}
	if v, ok := _u.mutation.Version(); ok {
		if err := incident.VersionValidator(v); err != nil {
			return &ValidationError{Name: "version", err: fmt.Errorf(`migrations: validator failed for field "Incident.version": %w`, err)}
		}
	}
	return nil
}
...
func (_u *IncidentUpdateOne) sqlSave(ctx context.Context) (_node *Incident, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "Incident.id" for update`)}
	}
