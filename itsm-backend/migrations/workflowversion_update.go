...
func (_u *WorkflowVersionUpdate) check() error {
	...
	if _u.mutation.WorkflowCleared() && len(_u.mutation.WorkflowIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "WorkflowVersion.workflow"`)
	}
	return nil
}
...
func (_u *WorkflowVersionUpdateOne) sqlSave(ctx context.Context) (_node *WorkflowVersion, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "WorkflowVersion.id" for update`)}
	}
