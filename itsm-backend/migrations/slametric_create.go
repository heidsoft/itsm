// Code generated by ent, DO NOT EDIT.

package migrations

import (
	"context"
	"errors"
	"fmt"
	"itsm-backend/migrations/sladefinition"
	"itsm-backend/migrations/slametric"
	"time"

	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
)

// SLAMetricCreate is the builder for creating a SLAMetric entity.
type SLAMetricCreate struct {
	config
	mutation *SLAMetricMutation
	hooks    []Hook
}

// SetSLADefinitionID sets the "sla_definition_id" field.
func (_c *SLAMetricCreate) SetSLADefinitionID(v int) *SLAMetricCreate {
	_c.mutation.SetSLADefinitionID(v)
	return _c
}

// SetMetricType sets the "metric_type" field.
func (_c *SLAMetricCreate) SetMetricType(v string) *SLAMetricCreate {
	_c.mutation.SetMetricType(v)
	return _c
}

// SetMetricName sets the "metric_name" field.
func (_c *SLAMetricCreate) SetMetricName(v string) *SLAMetricCreate {
	_c.mutation.SetMetricName(v)
	return _c
}

// SetMetricValue sets the "metric_value" field.
func (_c *SLAMetricCreate) SetMetricValue(v float64) *SLAMetricCreate {
	_c.mutation.SetMetricValue(v)
	return _c
}

// SetUnit sets the "unit" field.
func (_c *SLAMetricCreate) SetUnit(v string) *SLAMetricCreate {
	_c.mutation.SetUnit(v)
	return _c
}

// SetNillableUnit sets the "unit" field if the given value is not nil.
func (_c *SLAMetricCreate) SetNillableUnit(v *string) *SLAMetricCreate {
	if v != nil {
		_c.SetUnit(*v)
	}
	return _c
}

// SetMeasurementTime sets the "measurement_time" field.
func (_c *SLAMetricCreate) SetMeasurementTime(v time.Time) *SLAMetricCreate {
	_c.mutation.SetMeasurementTime(v)
	return _c
}

// SetNillableMeasurementTime sets the "measurement_time" field if the given value is not nil.
func (_c *SLAMetricCreate) SetNillableMeasurementTime(v *time.Time) *SLAMetricCreate {
	if v != nil {
		_c.SetMeasurementTime(*v)
	}
	return _c
}

// SetMetadata sets the "metadata" field.
func (_c *SLAMetricCreate) SetMetadata(v map[string]interface{}) *SLAMetricCreate {
	_c.mutation.SetMetadata(v)
	return _c
}

// SetTenantID sets the "tenant_id" field.
func (_c *SLAMetricCreate) SetTenantID(v int) *SLAMetricCreate {
	_c.mutation.SetTenantID(v)
	return _c
}

// SetCreatedAt sets the "created_at" field.
func (_c *SLAMetricCreate) SetCreatedAt(v time.Time) *SLAMetricCreate {
	_c.mutation.SetCreatedAt(v)
	return _c
}

// SetNillableCreatedAt sets the "created_at" field if the given value is not nil.
func (_c *SLAMetricCreate) SetNillableCreatedAt(v *time.Time) *SLAMetricCreate {
	if v != nil {
		_c.SetCreatedAt(*v)
	}
	return _c
}

// SetUpdatedAt sets the "updated_at" field.
func (_c *SLAMetricCreate) SetUpdatedAt(v time.Time) *SLAMetricCreate {
	_c.mutation.SetUpdatedAt(v)
	return _c
}

// SetNillableUpdatedAt sets the "updated_at" field if the given value is not nil.
func (_c *SLAMetricCreate) SetNillableUpdatedAt(v *time.Time) *SLAMetricCreate {
	if v != nil {
		_c.SetUpdatedAt(*v)
	}
	return _c
}

// SetSLADefinition sets the "sla_definition" edge to the SLADefinition entity.
func (_c *SLAMetricCreate) SetSLADefinition(v *SLADefinition) *SLAMetricCreate {
	return _c.SetSLADefinitionID(v.ID)
}

// Mutation returns the SLAMetricMutation object of the builder.
func (_c *SLAMetricCreate) Mutation() *SLAMetricMutation {
	return _c.mutation
}

// Save creates the SLAMetric in the database.
func (_c *SLAMetricCreate) Save(ctx context.Context) (*SLAMetric, error) {
	_c.defaults()
	return withHooks(ctx, _c.sqlSave, _c.mutation, _c.hooks)
}

// SaveX calls Save and panics if Save returns an error.
func (_c *SLAMetricCreate) SaveX(ctx context.Context) *SLAMetric {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *SLAMetricCreate) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *SLAMetricCreate) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}

// defaults sets the default values of the builder before save.
func (_c *SLAMetricCreate) defaults() {
	if _, ok := _c.mutation.MeasurementTime(); !ok {
		v := slametric.DefaultMeasurementTime()
		_c.mutation.SetMeasurementTime(v)
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		v := slametric.DefaultCreatedAt()
		_c.mutation.SetCreatedAt(v)
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		v := slametric.DefaultUpdatedAt()
		_c.mutation.SetUpdatedAt(v)
	}
}

// check runs all checks and user-defined validators on the builder.
func (_c *SLAMetricCreate) check() error {
	if _, ok := _c.mutation.SLADefinitionID(); !ok {
		return &ValidationError{Name: "sla_definition_id", err: errors.New(`migrations: missing required field "SLAMetric.sla_definition_id"`)}
	}
	if v, ok := _c.mutation.SLADefinitionID(); ok {
		if err := slametric.SLADefinitionIDValidator(v); err != nil {
			return &ValidationError{Name: "sla_definition_id", err: fmt.Errorf(`migrations: validator failed for field "SLAMetric.sla_definition_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.MetricType(); !ok {
		return &ValidationError{Name: "metric_type", err: errors.New(`migrations: missing required field "SLAMetric.metric_type"`)}
	}
	if v, ok := _c.mutation.MetricType(); ok {
		if err := slametric.MetricTypeValidator(v); err != nil {
			return &ValidationError{Name: "metric_type", err: fmt.Errorf(`migrations: validator failed for field "SLAMetric.metric_type": %w`, err)}
		}
	}
	if _, ok := _c.mutation.MetricName(); !ok {
		return &ValidationError{Name: "metric_name", err: errors.New(`migrations: missing required field "SLAMetric.metric_name"`)}
	}
	if v, ok := _c.mutation.MetricName(); ok {
		if err := slametric.MetricNameValidator(v); err != nil {
			return &ValidationError{Name: "metric_name", err: fmt.Errorf(`migrations: validator failed for field "SLAMetric.metric_name": %w`, err)}
		}
	}
	if _, ok := _c.mutation.MetricValue(); !ok {
		return &ValidationError{Name: "metric_value", err: errors.New(`migrations: missing required field "SLAMetric.metric_value"`)}
	}
	if _, ok := _c.mutation.MeasurementTime(); !ok {
		return &ValidationError{Name: "measurement_time", err: errors.New(`migrations: missing required field "SLAMetric.measurement_time"`)}
	}
	if _, ok := _c.mutation.TenantID(); !ok {
		return &ValidationError{Name: "tenant_id", err: errors.New(`migrations: missing required field "SLAMetric.tenant_id"`)}
	}
	if v, ok := _c.mutation.TenantID(); ok {
		if err := slametric.TenantIDValidator(v); err != nil {
			return &ValidationError{Name: "tenant_id", err: fmt.Errorf(`migrations: validator failed for field "SLAMetric.tenant_id": %w`, err)}
		}
	}
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`migrations: missing required field "SLAMetric.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`migrations: missing required field "SLAMetric.updated_at"`)}
	}
	if len(_c.mutation.SLADefinitionIDs()) == 0 {
		return &ValidationError{Name: "sla_definition", err: errors.New(`migrations: missing required edge "SLAMetric.sla_definition"`)}
	}
	return nil
}

func (_c *SLAMetricCreate) sqlSave(ctx context.Context) (*SLAMetric, error) {
	if err := _c.check(); err != nil {
		return nil, err
	}
	_node, _spec := _c.createSpec()
	if err := sqlgraph.CreateNode(ctx, _c.driver, _spec); err != nil {
		if sqlgraph.IsConstraintError(err) {
			err = &ConstraintError{msg: err.Error(), wrap: err}
		}
		return nil, err
	}
	id := _spec.ID.Value.(int64)
	_node.ID = int(id)
	_c.mutation.id = &_node.ID
	_c.mutation.done = true
	return _node, nil
}

func (_c *SLAMetricCreate) createSpec() (*SLAMetric, *sqlgraph.CreateSpec) {
	var (
		_node = &SLAMetric{config: _c.config}
		_spec = sqlgraph.NewCreateSpec(slametric.Table, sqlgraph.NewFieldSpec(slametric.FieldID, field.TypeInt))
	)
	if value, ok := _c.mutation.MetricType(); ok {
		_spec.SetField(slametric.FieldMetricType, field.TypeString, value)
		_node.MetricType = value
	}
	if value, ok := _c.mutation.MetricName(); ok {
		_spec.SetField(slametric.FieldMetricName, field.TypeString, value)
		_node.MetricName = value
	}
	if value, ok := _c.mutation.MetricValue(); ok {
		_spec.SetField(slametric.FieldMetricValue, field.TypeFloat64, value)
		_node.MetricValue = value
	}
	if value, ok := _c.mutation.Unit(); ok {
		_spec.SetField(slametric.FieldUnit, field.TypeString, value)
		_node.Unit = value
	}
	if value, ok := _c.mutation.MeasurementTime(); ok {
		_spec.SetField(slametric.FieldMeasurementTime, field.TypeTime, value)
		_node.MeasurementTime = value
	}
	if value, ok := _c.mutation.Metadata(); ok {
		_spec.SetField(slametric.FieldMetadata, field.TypeJSON, value)
		_node.Metadata = value
	}
	if value, ok := _c.mutation.TenantID(); ok {
		_spec.SetField(slametric.FieldTenantID, field.TypeInt, value)
		_node.TenantID = value
	}
	if value, ok := _c.mutation.CreatedAt(); ok {
		_spec.SetField(slametric.FieldCreatedAt, field.TypeTime, value)
		_node.CreatedAt = value
	}
	if value, ok := _c.mutation.UpdatedAt(); ok {
		_spec.SetField(slametric.FieldUpdatedAt, field.TypeTime, value)
		_node.UpdatedAt = value
	}
	if nodes := _c.mutation.SLADefinitionIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: true,
			Table:   slametric.SLADefinitionTable,
			Columns: []string{slametric.SLADefinitionColumn},
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
				IDSpec: sqlgraph.NewFieldSpec(sladefinition.FieldID, field.TypeInt),
			},
		}
		for _, k := range nodes {
			edge.Target.Nodes = append(edge.Target.Nodes, k)
		}
		_node.SLADefinitionID = nodes[0]
		_spec.Edges = append(_spec.Edges, edge)
	}
	return _node, _spec
}

// SLAMetricCreateBulk is the builder for creating many SLAMetric entities in bulk.
type SLAMetricCreateBulk struct {
	config
	err      error
	builders []*SLAMetricCreate
}

// Save creates the SLAMetric entities in the database.
func (_c *SLAMetricCreateBulk) Save(ctx context.Context) ([]*SLAMetric, error) {
	if _c.err != nil {
		return nil, _c.err
	}
	specs := make([]*sqlgraph.CreateSpec, len(_c.builders))
	nodes := make([]*SLAMetric, len(_c.builders))
	mutators := make([]Mutator, len(_c.builders))
	for i := range _c.builders {
		func(i int, root context.Context) {
			builder := _c.builders[i]
			builder.defaults()
			var mut Mutator = MutateFunc(func(ctx context.Context, m Mutation) (Value, error) {
				mutation, ok := m.(*SLAMetricMutation)
				if !ok {
					return nil, fmt.Errorf("unexpected mutation type %T", m)
				}
				if err := builder.check(); err != nil {
					return nil, err
				}
				builder.mutation = mutation
				var err error
				nodes[i], specs[i] = builder.createSpec()
				if i < len(mutators)-1 {
					_, err = mutators[i+1].Mutate(root, _c.builders[i+1].mutation)
				} else {
					spec := &sqlgraph.BatchCreateSpec{Nodes: specs}
					// Invoke the actual operation on the latest mutation in the chain.
					if err = sqlgraph.BatchCreate(ctx, _c.driver, spec); err != nil {
						if sqlgraph.IsConstraintError(err) {
							err = &ConstraintError{msg: err.Error(), wrap: err}
						}
					}
				}
				if err != nil {
					return nil, err
				}
				mutation.id = &nodes[i].ID
				if specs[i].ID.Value != nil {
					id := specs[i].ID.Value.(int64)
					nodes[i].ID = int(id)
				}
				mutation.done = true
				return nodes[i], nil
			})
			for i := len(builder.hooks) - 1; i >= 0; i-- {
				mut = builder.hooks[i](mut)
			}
			mutators[i] = mut
		}(i, ctx)
	}
	if len(mutators) > 0 {
		if _, err := mutators[0].Mutate(ctx, _c.builders[0].mutation); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

// SaveX is like Save, but panics if an error occurs.
func (_c *SLAMetricCreateBulk) SaveX(ctx context.Context) []*SLAMetric {
	v, err := _c.Save(ctx)
	if err != nil {
		panic(err)
	}
	return v
}

// Exec executes the query.
func (_c *SLAMetricCreateBulk) Exec(ctx context.Context) error {
	_, err := _c.Save(ctx)
	return err
}

// ExecX is like Exec, but panics if an error occurs.
func (_c *SLAMetricCreateBulk) ExecX(ctx context.Context) {
	if err := _c.Exec(ctx); err != nil {
		panic(err)
	}
}
