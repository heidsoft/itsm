...
func (_q *ApplicationQuery) QueryProject() *ProjectQuery {
...
func (_q *ApplicationQuery) QueryMicroservices() *MicroserviceQuery {
...
func (_q *ApplicationQuery) QueryTags() *TagQuery {
...
func (_q *ApplicationQuery) WithProject(opts ...func(*ProjectQuery)) *ApplicationQuery {
	query := (&ProjectClient{config: _q.config}).Query()
...
func (_q *ApplicationQuery) WithMicroservices(opts ...func(*MicroserviceQuery)) *ApplicationQuery {
	query := (&MicroserviceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) WithTags(opts ...func(*TagQuery)) *ApplicationQuery {
	query := (&TagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ApplicationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Application, error) {
	...
	if query := _q.withMicroservices; query != nil {
		if err := _q.loadMicroservices(ctx, query, nodes,
			func(n *Application) { n.Edges.Microservices = []*Microservice{} },
			func(n *Application, e *Microservice) { n.Edges.Microservices = append(n.Edges.Microservices, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *ApplicationQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*Application, init func(*Application), assign func(*Application, *Project)) error {
...
func (_q *ApplicationQuery) loadMicroservices(ctx context.Context, query *MicroserviceQuery, nodes []*Application, init func(*Application), assign func(*Application, *Microservice)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(microservice.FieldApplicationID)
	}
	query.Where(predicate.Microservice(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(application.MicroservicesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *ApplicationQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Application, init func(*Application), assign func(*Application, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(application.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(application.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(application.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(application.TagsPrimaryKey[0]))
