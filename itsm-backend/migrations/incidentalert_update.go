...
func (_u *IncidentAlertUpdate) check() error {
	...
	if _u.mutation.IncidentCleared() && len(_u.mutation.IncidentIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "IncidentAlert.incident"`)
	}
	return nil
}
...
func (_u *IncidentAlertUpdateOne) sqlSave(ctx context.Context) (_node *IncidentAlert, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "IncidentAlert.id" for update`)}
	}
