...
func (_u *IncidentMetricUpdate) check() error {
	...
	if _u.mutation.IncidentCleared() && len(_u.mutation.IncidentIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "IncidentMetric.incident"`)
	}
	return nil
}
...
func (_u *IncidentMetricUpdateOne) sqlSave(ctx context.Context) (_node *IncidentMetric, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "IncidentMetric.id" for update`)}
	}
