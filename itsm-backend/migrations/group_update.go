...
func (_u *GroupUpdate) check() error {
	if v, ok := _u.mutation.Name(); ok {
		if err := group.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`migrations: validator failed for field "Group.name": %w`, err)}
		}
	}
	if v, ok := _u.mutation.TenantID(); ok {
		if err := group.TenantIDValidator(v); err != nil {
			return &ValidationError{Name: "tenant_id", err: fmt.Errorf(`migrations: validator failed for field "Group.tenant_id": %w`, err)}
		}
	}
	return nil
}
...
func (_u *GroupUpdateOne) sqlSave(ctx context.Context) (_node *Group, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "Group.id" for update`)}
	}
