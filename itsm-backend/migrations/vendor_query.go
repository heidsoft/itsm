...
func (_q *VendorQuery) QueryContracts() *ContractQuery {
...
func (_q *VendorQuery) QueryAssets() *AssetQuery {
...
func (_q *VendorQuery) WithContracts(opts ...func(*ContractQuery)) *VendorQuery {
	query := (&ContractClient{config: _q.config}).Query()
...
func (_q *VendorQuery) WithAssets(opts ...func(*AssetQuery)) *VendorQuery {
	query := (&AssetClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *VendorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vendor, error) {
	...
	if query := _q.withContracts; query != nil {
		if err := _q.loadContracts(ctx, query, nodes,
			func(n *Vendor) { n.Edges.Contracts = []*Contract{} },
			func(n *Vendor, e *Contract) { n.Edges.Contracts = append(n.Edges.Contracts, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withAssets; query != nil {
		if err := _q.loadAssets(ctx, query, nodes,
			func(n *Vendor) { n.Edges.Assets = []*Asset{} },
			func(n *Vendor, e *Asset) { n.Edges.Assets = append(n.Edges.Assets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *VendorQuery) loadContracts(ctx context.Context, query *ContractQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *Contract)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(contract.FieldVendorID)
	}
	query.Where(predicate.Contract(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.ContractsColumn), fks...))
	}))
...
func (_q *VendorQuery) loadAssets(ctx context.Context, query *AssetQuery, nodes []*Vendor, init func(*Vendor), assign func(*Vendor, *Asset)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Vendor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Asset(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(vendor.AssetsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.vendor_assets
		if fk == nil {
			return fmt.Errorf(`foreign-key "vendor_assets" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "vendor_assets" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
