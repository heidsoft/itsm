...
func (_u *CloudResourceUpdate) check() error {
	...
	if _u.mutation.AccountCleared() && len(_u.mutation.AccountIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "CloudResource.account"`)
	}
	if _u.mutation.ServiceCleared() && len(_u.mutation.ServiceIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "CloudResource.service"`)
	}
	return nil
}
...
func (_u *CloudResourceUpdateOne) sqlSave(ctx context.Context) (_node *CloudResource, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "CloudResource.id" for update`)}
	}
