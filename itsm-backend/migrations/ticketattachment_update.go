...
func (_u *TicketAttachmentUpdate) check() error {
	...
	if _u.mutation.TicketCleared() && len(_u.mutation.TicketIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "TicketAttachment.ticket"`)
	}
	if _u.mutation.UploaderCleared() && len(_u.mutation.UploaderIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "TicketAttachment.uploader"`)
	}
	return nil
}
...
func (_u *TicketAttachmentUpdateOne) sqlSave(ctx context.Context) (_node *TicketAttachment, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "TicketAttachment.id" for update`)}
	}
