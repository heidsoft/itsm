...
func (_u *ProcessInstanceUpdate) check() error {
	...
	if _u.mutation.DefinitionCleared() && len(_u.mutation.DefinitionIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "ProcessInstance.definition"`)
	}
	return nil
}
...
func (_u *ProcessInstanceUpdateOne) sqlSave(ctx context.Context) (_node *ProcessInstance, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "ProcessInstance.id" for update`)}
	}
