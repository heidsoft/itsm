...
func (_u *TenantUpdate) check() error {
	if v, ok := _u.mutation.Name(); ok {
		if err := tenant.NameValidator(v); err != nil {
			return &ValidationError{Name: "name", err: fmt.Errorf(`migrations: validator failed for field "Tenant.name": %w`, err)}
		}
	}
	if v, ok := _u.mutation.Code(); ok {
		if err := tenant.CodeValidator(v); err != nil {
			return &ValidationError{Name: "code", err: fmt.Errorf(`migrations: validator failed for field "Tenant.code": %w`, err)}
		}
	}
	if v, ok := _u.mutation.GetType(); ok {
		if err := tenant.TypeValidator(v); err != nil {
			return &ValidationError{Name: "type", err: fmt.Errorf(`migrations: validator failed for field "Tenant.type": %w`, err)}
		}
	}
	return nil
}
...
func (_u *TenantUpdateOne) sqlSave(ctx context.Context) (_node *Tenant, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "Tenant.id" for update`)}
	}
