...
func (_u *ServiceRequestUpdate) check() error {
	if v, ok := _u.mutation.TenantID(); ok {
		if err := servicerequest.TenantIDValidator(v); err != nil {
			return &ValidationError{Name: "tenant_id", err: fmt.Errorf(`migrations: validator failed for field "ServiceRequest.tenant_id": %w`, err)}
		}
	}
	if v, ok := _u.mutation.CatalogID(); ok {
		if err := servicerequest.CatalogIDValidator(v); err != nil {
			return &ValidationError{Name: "catalog_id", err: fmt.Errorf(`migrations: validator failed for field "ServiceRequest.catalog_id": %w`, err)}
		}
	}
	if v, ok := _u.mutation.RequesterID(); ok {
		if err := servicerequest.RequesterIDValidator(v); err != nil {
			return &ValidationError{Name: "requester_id", err: fmt.Errorf(`migrations: validator failed for field "ServiceRequest.requester_id": %w`, err)}
		}
	}
	return nil
}
...
func (_u *ServiceRequestUpdateOne) sqlSave(ctx context.Context) (_node *ServiceRequest, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "ServiceRequest.id" for update`)}
	}
