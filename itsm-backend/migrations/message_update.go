...
func (_u *MessageUpdate) check() error {
	if _u.mutation.ConversationCleared() && len(_u.mutation.ConversationIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "Message.conversation"`)
	}
	return nil
}
...
func (_u *MessageUpdateOne) sqlSave(ctx context.Context) (_node *Message, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "Message.id" for update`)}
	}
