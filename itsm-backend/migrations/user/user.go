...
const (
	...
	// PirReviewsTable is the table that holds the pir_reviews relation/edge. The primary key declared below.
	PirReviewsTable = "user_pir_reviews"
...
var (
	...
	// PirReviewsPrimaryKey and PirReviewsColumn2 are the table columns denoting the
	// primary key for the pir_reviews relation (M2M).
	PirReviewsPrimaryKey = []string{"user_id", "change_pir_id"}
)
...
func newPirReviewsStep() *sqlgraph.Step {
	return sqlgraph.NewStep(
		...
		sqlgraph.Edge(sqlgraph.M2M, false, PirReviewsTable, PirReviewsPrimaryKey...),
	)
}
