...
func (_u *TicketNotificationUpdate) check() error {
	...
	if _u.mutation.TicketCleared() && len(_u.mutation.TicketIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "TicketNotification.ticket"`)
	}
	if _u.mutation.UserCleared() && len(_u.mutation.UserIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "TicketNotification.user"`)
	}
	return nil
}
...
func (_u *TicketNotificationUpdateOne) sqlSave(ctx context.Context) (_node *TicketNotification, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "TicketNotification.id" for update`)}
	}
