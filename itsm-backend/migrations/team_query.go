...
func (_q *TeamQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Team, init func(*Team), assign func(*Team, *User)) error {
	...
	for _, n := range neighbors {
		fk := n.team_users
		if fk == nil {
			return fmt.Errorf(`foreign-key "team_users" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "team_users" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *TeamQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Team, init func(*Team), assign func(*Team, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(team.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(team.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(team.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(team.TagsPrimaryKey[0]))
