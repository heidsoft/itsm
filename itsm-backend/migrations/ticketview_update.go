...
func (_u *TicketViewUpdate) check() error {
	...
	if _u.mutation.CreatorCleared() && len(_u.mutation.CreatorIDs()) > 0 {
		return errors.New(`migrations: clearing a required unique edge "TicketView.creator"`)
	}
	return nil
}
...
func (_u *TicketViewUpdateOne) sqlSave(ctx context.Context) (_node *TicketView, err error) {
	...
	if !ok {
		return nil, &ValidationError{Name: "id", err: errors.New(`migrations: missing "TicketView.id" for update`)}
	}
