...
// 优先使用 AuditLogController（支持过滤/分页，返回 {logs,total,page,pageSize} 契约），
...
func SetupAuditLogRoutes(
	...
	if auditLogController != nil {
		tenant.GET("/audit-logs", middleware.RequirePermission("audit", "read"), auditLogController.ListAuditLogs)
