...
type UpdateLicenseRequest struct {
	...
	LicenseType    *string  `json:"licenseType"`    // 许可证类型
	TotalQuantity  *int     `json:"totalQuantity"`  // 总数量
	UsedQuantity   *int     `json:"usedQuantity"`   // 已使用数量
	AssetID        *int     `json:"assetId"`        // 关联的资产ID
	PurchaseDate   *string  `json:"purchaseDate"`   // 采购日期
	PurchasePrice  *float64 `json:"purchasePrice"`  // 采购价格
	ExpiryDate     *string  `json:"expiryDate"`     // 到期日期
...
type LicenseResponse struct {
	...
	LicenseKey        string    `json:"licenseKey"`        // 许可证密钥
	Name              string    `json:"name"`              // 许可证名称
	Description       string    `json:"description"`       // 许可证描述
	Vendor            string    `json:"vendor"`            // 供应商
	LicenseType       string    `json:"licenseType"`       // 许可证类型
	TotalQuantity     int       `json:"totalQuantity"`     // 总数量
	UsedQuantity      int       `json:"usedQuantity"`      // 已使用数量
	AvailableQuantity int       `json:"availableQuantity"` // 可用数量
	TenantID          int       `json:"tenantId"`          // 租户ID
	AssetID           *int      `json:"assetId"`           // 关联的资产ID
	AssetName         *string   `json:"assetName"`         // 关联的资产名称
	PurchaseDate      string    `json:"purchaseDate"`      // 采购日期
	PurchasePrice     *float64  `json:"purchasePrice"`     // 采购价格
	ExpiryDate        string    `json:"expiryDate"`        // 到期日期
	SupportVendor     string    `json:"supportVendor"`     // 支持供应商
	...
	Notes             string    `json:"notes"`             // 备注
	Users             []int     `json:"users"`             // 授权用户列表
	UserNames         []string  `json:"userNames"`         // 授权用户姓名列表
	Tags              []string  `json:"tags"`              // 标签
...
type LicenseStatsResponse struct {
	...
	ExpiringSoon   int     `json:"expiringSoon"`   // 即将过期
	Depleted       int     `json:"depleted"`       // 已耗尽
	ComplianceRate float64 `json:"complianceRate"` // 合规率
}

// LicenseAssignRequest 许可证分配请求
type LicenseAssignRequest struct {
	UserIDs []int   `json:"userIds" binding:"required"` // 用户ID列表
	Comment *string `json:"comment"`                    // 分配说明
}

// LicenseRenewalRequest 许可证续期请求
type LicenseRenewalRequest struct {
	NewExpiryDate string  `json:"newExpiryDate" binding:"required"` // 新到期日期
	RenewalCost   string  `json:"renewalCost"`                      // 续费成本
	Vendor        string  `json:"vendor"`                           // 供应商
