...
type TicketViewFilter struct {
	...
	DateFrom    string            `json:"dateFrom,omitempty"`  // ISO 日期
	DateTo      string            `json:"dateTo,omitempty"`    // ISO 日期
	SLAStatus   []string          `json:"slaStatus,omitempty"` // SLA状态过滤
	TagIDs      []int             `json:"tagIds,omitempty"`    // 标签ID过滤
