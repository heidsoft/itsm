...
// generateNextVersion 生成下一个版本号（major.minor.0）。
// 约定：递增 minor 段、patch 归零，minor 不封顶（允许 1.10.0 等）。
// 旧实现仅识别 1.0.0~1.3.0，超出后回退 "1.0.0"，与唯一索引 (tenant_id,key,version) 冲突导致部署失败/覆盖；此处改为正确递增。
