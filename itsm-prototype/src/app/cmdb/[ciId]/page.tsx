...
import { Zap, ArrowLeft, Network, Server, Database, Cloud, AlertTriangle } from 'lucide-react';
...
const mockCIDetail = {
  'CI-ECS-001': {
    ...
    description:
      'Alibaba Cloud ECS instance hosting the e-commerce platform frontend web services, located in East China 1 Availability Zone J.',
    attributes: {
    ...
    relatedTickets: [
      {
        id: 'INC-00125',
        type: 'Incident',
        title: 'Web server CPU usage exceeds 95%',
        status: 'In Progress',
      },
      {
        id: 'PRB-00003',
        type: 'Problem',
        title: 'Database connection pool exhaustion causing application crash',
        status: 'Known Error',
      },
      {
        id: 'CHG-00003',
        type: 'Change',
        title: 'Emergency fix for web server security vulnerability',
        status: 'Implementing',
      },
    ],
    relationships: [
      ...
      {
        targetId: 'CI-RDS-001',
        targetName: 'CRM Database - Production Environment',
        type: 'Connects to',
      },
    ],
    impacts: [
      ...
      {
        id: 'SLA-001',
        name: 'Production CRM System Availability',
        type: 'SLA',
        impactLevel: 'High',
      },
    ],
  ...
  'CI-RDS-001': {
    ...
    owner: 'DBA Team',
    description:
      'MySQL database instance used by the e-commerce platform CRM system, configured for high availability.',
    attributes: {
    ...
    relatedTickets: [
      {
        id: 'INC-00122',
        type: 'Incident',
        title: 'Production database master-slave synchronization delay',
        status: 'Resolved',
      },
    ],
    relationships: [
      {
        targetId: 'CI-ECS-001',
        targetName: 'Web Server - Production Environment',
        type: 'Connected by',
      },
    ],
    impacts: [
  ...
  'CI-APP-CRM': {
    ...
    relatedTickets: [
      {
        id: 'INC-00124',
        type: 'Incident',
        title: 'Users report unable to access CRM system',
        status: 'Assigned',
      },
      {
        id: 'PRB-00001',
        type: 'Problem',
        title: 'CRM system intermittent login failures',
    ...
    impacts: [
      ...
      {
        id: 'REQ-00101',
        name: 'User unable to login to CRM',
        ...
        <div className='lg:col-span-2 bg-white p-8 rounded-lg shadow-md'>
          ...
          <div className='bg-gray-50 p-6 rounded-lg border border-gray-200'>
            <p className='text-gray-600'>
              This area will display the relationship diagram between configuration items, for
              example:
            </p>
            <ul className='list-disc list-inside text-gray-700 mt-2'>
              {ci.relationships.map((rel, i) => (
                <li key={i}>
                  {ci.name} <span className='font-semibold text-blue-600'>{rel.type}</span>{' '}
                  <Link href={`/cmdb/${rel.targetId}`} className='text-blue-600 hover:underline'>
                    {rel.targetName}
                  </Link>
                </li>
              ))}
            ...
            <p className='text-sm text-gray-500 mt-4'>
              (In the actual product, this would be an interactive relationship topology diagram)
            </p>
          </div>
        </div>
        ...
        <div className='space-y-8'>
          <div className='bg-white p-6 rounded-lg shadow-md'>
            ...
            <div className='space-y-3 text-sm'>
              <div className='flex justify-between'>
                <span>Type:</span>
                <span className='font-semibold'>{ci.type}</span>
              </div>
              <div className='flex justify-between'>
                <span>Status:</span>
                <span className='font-semibold'>{ci.status}</span>
              </div>
              <div className='flex justify-between'>
                <span>Business:</span>
                <span className='font-semibold'>{ci.business}</span>
              </div>
              <div className='flex justify-between'>
                <span>Owner:</span>
                <span className='font-semibold'>{ci.owner}</span>
              </div>
            </div>
          </div>
          ...
          <div className='bg-white p-6 rounded-lg shadow-md'>
            ...
            <div className='space-y-3'>
              {ci.relatedTickets.length > 0 ? (
                ci.relatedTickets.map(ticket => (
                  <Link
                    key={ticket.id}
                    href={`/${
