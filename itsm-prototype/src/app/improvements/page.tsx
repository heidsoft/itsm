...
const mockImprovements = [
    { id: 'IMP-001', title: '优化云资源申请流程', status: '进行中', owner: '产品部', target: '将云资源交付时间缩短20%', createdAt: '2025-05-01' },
    { id: 'IMP-002', title: '提升服务台首次解决率', status: '待评估', owner: '服务台经理', target: '将首次解决率提升至85%', createdAt: '2025-04-10' },
    { id: 'IMP-003', title: '自动化安全漏洞扫描与修复', status: '已完成', owner: '安全团队', target: '自动化覆盖率达到90%', createdAt: '2025-03-01' },
];
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">计划ID</th>
                        ...
                        {mockImprovements.map(imp => (
                            <tr key={imp.id} className="hover:bg-gray-50">
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <Link href={`/improvements/${imp.id}`} className="text-blue-600 font-semibold hover:underline">
                                        {imp.id}
                                    </Link>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap max-w-sm truncate">{imp.title}</td>
                                <td className="px-6 py-4 whitespace-nowrap"><ImprovementStatusBadge status={imp.status} /></td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{imp.owner}</td>
                                <td className="px-6 py-4 whitespace-nowrap max-w-sm truncate text-sm text-gray-700">{imp.target}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{imp.createdAt}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
