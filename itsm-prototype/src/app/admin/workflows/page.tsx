...
const WorkflowManagement = () => {
  ...
  const stats = {
    ...
    avgSteps: Math.round(
      workflows.reduce((sum, w) => sum + w.stepsCount, 0) / workflows.length
    ),
  };

  // 获取所有工作流类型
  const workflowTypes = Array.from(
    new Set(workflows.map((workflow) => workflow.type))
  );
  ...
  const handleBatchDelete = () => {
    setWorkflows((prev) =>
      prev.filter((workflow) => !selectedRowKeys.includes(workflow.id))
    );
    setSelectedRowKeys([]);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (type: string) => (
        <Tag
          color={
            WORKFLOW_TYPE_CONFIG[type as keyof typeof WORKFLOW_TYPE_CONFIG]
              ?.color
          }
          icon={
            WORKFLOW_TYPE_CONFIG[type as keyof typeof WORKFLOW_TYPE_CONFIG]
      ...
      render: (_: any, record: Workflow) => {
        const completionRate =
          record.completedInstances > 0
            ? Math.round(
                (record.completedInstances /
                  (record.completedInstances + record.activeInstances)) *
                  100
              )
            : 0;
        return (
    ...
    },
    {
      ...
      render: (_: any, record: Workflow) => (
        <Space>
          ...
          <Tooltip
            title={record.status === WORKFLOW_STATUS.ACTIVE ? "停用" : "启用"}
          >
            <Button
              type="text"
              icon={
          ...
          <Row gutter={16}>
            ...
            <Col span={12}>
              <Form.Item
                label="状态"
                name="status"
                initialValue={WORKFLOW_STATUS.DRAFT}
              >
                <Select>
                  {Object.entries(STATUS_CONFIG).map(([key, config]) => (
                    <Option key={key} value={key}>
              ...
              <Row gutter={[16, 8]}>
                ...
                <Col span={12}>
                  ...
                  <Tag
                    color={
                      STATUS_CONFIG[
                        selectedWorkflow.status as keyof typeof STATUS_CONFIG
                      ]?.color
                    }
                    icon={
                      STATUS_CONFIG[
