...
const EscalationRuleManagement = () => {
  ...
  const serviceTypes = Array.from(
    new Set(escalationRules.map((rule) => rule.serviceType))
  );
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  {level.notificationMethod.map((method) => (
                    <Tag
                      ...
                      color={
                        NOTIFICATION_CONFIG[
                          method as keyof typeof NOTIFICATION_CONFIG
                        ]?.color
                      }
                      icon={
                        NOTIFICATION_CONFIG[
      ...
      render: (_: any, record: EscalationRule) => (
        <div>
          ...
          <div className="flex items-center gap-4 mt-1">
            ...
            <span className="text-xs text-gray-500">
              级别: {record.escalationLevels.length}
            </span>
          </div>
        </div>
      ),
      ...
      >
        <Form form={form} layout="vertical" className="mt-4">
          ...
          >
            <Input.TextArea
              rows={2}
              placeholder="例如：工单状态为'进行中'且优先级为P1"
            />
          </Form.Item>
        </Form>
      </Modal>
            ...
            <Descriptions title="基本信息" bordered column={1} size="small">
              ...
              <Descriptions.Item label="状态">
                <Tag
                  color={
                    STATUS_CONFIG[
                      selectedRule.status as keyof typeof STATUS_CONFIG
                    ]?.color
                  }
                  icon={
                    STATUS_CONFIG[
