...
import { AlertTriangle, Shield, User, Cpu, Tag, Filter, Clock, PlusCircle, Zap } from 'lucide-react';
        ...
        <div className="p-10 bg-gray-50 min-h-full">
            ...
            <div className="flex flex-wrap items-center mb-6 bg-white p-3 rounded-lg shadow-sm gap-4">
                ...
                <div className="flex items-center ml-4">
                    ...
                    />
                    <label htmlFor="showMajorIncidents" className="ml-2 text-sm font-medium text-gray-700">只显示重大事件</label>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">事件ID</th>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredIncidents.length > 0 ? filteredIncidents.map(inc => (
                            <tr key={inc.id} className={`hover:bg-gray-50 ${inc.isMajorIncident ? 'bg-red-50/50 border-l-4 border-red-500' : ''}`}>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <Link href={`/incidents/${inc.id}`} className="text-blue-600 font-semibold hover:underline flex items-center">
                                        {inc.id}
                                        {inc.isMajorIncident && <Zap className="w-4 h-4 ml-2 text-red-600" title="重大事件" />}
                                    </Link>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap"><PriorityBadge priority={inc.priority} /></td>
                                <td className="px-6 py-4 whitespace-nowrap max-w-sm truncate">{inc.title}</td>
                                <td className="px-6 py-4 whitespace-nowrap"><StatusBadge status={inc.status} /></td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{inc.lastUpdate}</td>
                            </tr>
                        )) : (
                            <tr>
