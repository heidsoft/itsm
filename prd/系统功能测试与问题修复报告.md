...
- 后端返回完整的`Ticket`对象，前端期望`{ message, ticket_id }`
- 前端请求格式与后端DTO不匹配

**修复方案**:

- 修改前端`createTicket`返回类型为`Ticket`
- 修改前端`CreateTicketRequest`接口以匹配后端DTO
- 修改前端调用代码，转换格式
...
**详细修复报告**: 参见 `prd/前后端集成测试与修复报告.md`

---

**测试开始时间**：2025-12-07  
**测试人员**：AI Assistant  
**测试工具**：浏览器MCP自动化测试 + 代码审查 + PostgreSQL MCP
