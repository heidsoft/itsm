...
| 任务 | 优先级 | 文件范围 | 验收 |
|------|--------|----------|------|
| 补 `service/incident_service_test.go` 单测 | P0 | itsm-backend/service/incident*.go | 覆盖率 ≥ 60% |
| 补 `service/ticket_*_service_test.go` | P0 | itsm-backend/service/ticket*.go | 覆盖率 ≥ 60% |
| 补 `controller/auth_controller_test.go` 表驱动测试 | P0 | itsm-backend/controller/auth_controller.go | 覆盖率 ≥ 70% |
| 补 `service/sla_*_service_test.go` | P1 | itsm-backend/service/sla*.go | 覆盖率 ≥ 50% |
| 补 `service/escalation_*_service_test.go` | P1 | itsm-backend/service/escalation*.go | 覆盖率 ≥ 50% |
| **CI 门槛：1% → 40%** | P0 | `.github/workflows/ga-gate.yml` | 当 PR coverage < 40% 时 fail |
| 36 个 npm 漏洞（8 high + 26 moderate）评估与修复 | P1 | `itsm-frontend/package.json` + `npm audit fix` | 高危清零 |
| 5 处遗留 TODO 注释清理 | P2 | `process-routing` API 对接、菜单 key 重复 | grep TODO 为空 |
