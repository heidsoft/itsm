...
| 项 | 值 | 来源 |
|---|---|---|
| `itsm-backend/go.mod` 中 `go` 指令 | `1.25.12` | `itsm-backend/go.mod:3` |
| CI 安装版本 | `1.25.12`（按 `go-version-file` 自动跟随） | `.github/workflows/backend-ci.yml` 多处 `actions/setup-go@v7` |
| GH Actions 公共 README badge | `Go 1.25.12` | `README.md:9` |
| Dockerfile.prod 基础镜像 | `mirror.gcr.io/library/golang:1.25.12-alpine` | `itsm-backend/Dockerfile.prod:16` |
| Dockerfile dev 基础镜像 | `golang:1.25.12-alpine` | `itsm-backend/Dockerfile:18` |
...
`itsm-backend/go.mod` 顶部声明 `go 1.25.12`。Go 1.21+ 引入 **toolchain directive**：

- 当本地 `go version` < `go.mod` 要求时，**不设置 GOTOOLCHAIN** 会直接拒绝构建（"go.mod requires go >= 1.25.12 (running go 1.25.6; GOTOOLCHAIN=local)"）。
- 设置 `GOTOOLCHAIN=auto` 后，Go 工具链会自动下载并切换到所需版本（前提：本机 `go` ≥ 1.21，且网络可达 `proxy.golang.org` 或 `goproxy.cn`）。

CI 端通过 `actions/setup-go@v7` + `go-version-file: itsm-backend/go.mod` 直接安装 1.25.12，无需 `GOTOOLCHAIN`；但本地开发体验不同时，**`GOTOOLCHAIN=auto` 是契约**。
...
| **Docker** | `Dockerfile.prod` / `Dockerfile` | 已固定 `golang:1.25.12-alpine`，无需 `GOTOOLCHAIN` |
...
.github/workflows/backend-ci.yml
  → go-version-file: itsm-backend/go.mod  → 安装 1.25.12

# 2. 本地链路（模拟本地 1.25.6）
go version                     # go1.25.6 darwin/arm64
cd itsm-backend && GOTOOLCHAIN=auto go version
# → go1.25.12 (toolchain auto-downloaded)
...
|---|---|---|---|
| `>= 1.25.12` | ✅ | ✅ | ✅ |
| `1.21 ~ 1.25.11` | ❌ 报工具链错误 | ❌ 报工具链错误 | ✅ 自动下载 1.25.12 |
