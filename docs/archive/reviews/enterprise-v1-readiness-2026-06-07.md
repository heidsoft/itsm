...
- 使用本机 Docker 对 `prod` 栈完成初始化与重启验证
- `itsm-init` 在新版二进制下可正常 `exit 0`
- `itsm-backend` 在 `itsm-init` 之后启动
- `POST /api/v1/auth/login` 的 cookie 已修正为 host-only + `HttpOnly`
- 使用真实 cookie 访问 `/api/v1/auth/me` 与 `/api/v1/auth/menus` 成功
- 按数据库菜单扫描主页面路径，当前主导航路径全部返回 `200`
- 使用真实 token 成功创建工单，并可通过详情接口读取
- 使用真实 token 成功为工单创建评论，验证基础协作链路可用

仍待完成：

- `private`、`saas`、`saas_msp` 三种模式分别做一轮独立初始化验收
- 生产镜像源码构建在当前 Docker builder 中仍受内存限制影响，尚未完全通过
...
- 登录 + `me` + `menus` 主链路已完成
- 数据库菜单主路径页面可达性已完成第一轮扫描

当前剩余重点转向：

1. 三种部署模式的独立初始化验收
2. 生产镜像源码构建的资源约束优化
3. 页面级更深的流程烟雾测试，而不只是可达性测试
