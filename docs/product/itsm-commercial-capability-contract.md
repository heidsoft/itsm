...
|---|---|---|---|
| 工单/事件 | GA 候选 | 事件状态机、CI 多选关联、优先级矩阵、重大事件、BPMN 触发、租户过滤 | 创建后异步启动 BPMN 使用进程内 goroutine，失败只写日志；监控告警到事件的来源治理仍需验收 |
...
| BPMN/审批 | Pilot | 流程定义、绑定、实例、任务、变量、历史和多个 ITIL 域触发入口 | 多个业务服务以 goroutine 触发，业务落库与流程启动不原子；需要持久化 outbox/command 与补偿机制 |
...
1. 将业务落库后的 BPMN/通知触发从 goroutine 收敛为持久化 outbox/worker；
