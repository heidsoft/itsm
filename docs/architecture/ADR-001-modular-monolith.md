...
> **2026-09-12 注记**（追加于原文之上，未改动下方任何原文）
>
> 本 ADR 的**决策仍然有效**，状态维持 `Accepted` 而非 `superseded`：模块化单体、
> PostgreSQL 可靠命令基座、BPMN 单一编排层、禁止 goroutine fire-and-forget 等
> 核心判断均未改变。
>
> 仅**「背景」段的现状快照已过时**。原文称「当前系统同时存在 `controller/ + service/`
> 横向分层和 `handlers/<domain>/` 领域切片」，截至 2026-09-12 的实际情况是：
>
> - `controller/` 已清空（仅余空目录 `marketplace/`），**迁移期双轨结束**，
>   新代码统一进入 `handlers/<domain>/`（当前 66 个业务域，另有 `common/`、`shared/` 两个共享包）。
> - `service/` 仍然保留（291 个 `.go`），但已从「HTTP façade 的下游」转变为
>   跨域共享业务逻辑层，不再是 `controller/` 的配套分层。
>
> 按 ADR 惯例，历史背景与决策原文不作回改，以免伪造决策时的语境；现状差异以本注记存档。
> 落地证据与后续缺口见 [ROADMAP.md](../../ROADMAP.md) 的 v1.6.x 已落地与当前收敛项。
