...
export const TicketWorkflowActions: React.FC<TicketWorkflowActionsProps> = ({
  ...
  const [users, setUsers] = useState<any[]>([]); // User type from API

  // 加载用户列表
  React.useEffect(() => {
    if (
      modalVisible &&
      (currentAction === TicketWorkflowAction.FORWARD || currentAction === TicketWorkflowAction.CC)
    ) {
      loadUsers();
    }
              ...
              children: (
                <div>
                  ...
                  <div className='text-xs text-gray-400 mt-1'>
                    {approval.processedAt && new Date(approval.processedAt).toLocaleString('zh-CN')}
                  </div>
                </div>
              ),
            }))}
