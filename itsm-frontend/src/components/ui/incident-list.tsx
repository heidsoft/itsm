...
const IncidentFilters: React.FC<{
  ...
  const handleFilterChange = (
    key: keyof ListIncidentsRequest,
    ...
    <div className='bg-white p-4 rounded-lg shadow-sm border border-gray-200 mb-6'>
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4'>
        ...
        <div>
          ...
          <select
            value={
              filters.is_major_incident === undefined ? '' : filters.is_major_incident.toString()
            }
            onChange={e =>
              handleFilterChange(
                'is_major_incident',
    ...
    <tr className='hover:bg-gray-50 cursor-pointer' onClick={() => onSelect?.(incident)}>
      <td className='px-6 py-4 whitespace-nowrap'>
        <div className='flex items-center'>
          <div>
            <div className='text-sm font-medium text-gray-900'>{incident.incident_number}</div>
