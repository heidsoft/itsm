...
import { SearchOutlined, FileTextOutlined, AlertOutlined, QuestionCircleOutlined, SwapOutlined, BookOutlined } from '@ant-design/icons';
          ...
          renderItem={(item) => (
            ...
            >
              <List.Item.Meta
                ...
                title={
                  <span>
                    {item.ticketNumber && <span className={styles.ticketNumber}>{item.ticketNumber}</span>}
                    {item.title}
                  </span>
                }
                description={item.description?.slice(0, 60) + (item.description && item.description.length > 60 ? '...' : '')}
              />
