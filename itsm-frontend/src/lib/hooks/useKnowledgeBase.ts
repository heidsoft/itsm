...
import type { ArticleQuery, KnowledgeSearchRequest } from '@/types/knowledge-base';
...
export const KNOWLEDGE_BASE_KEYS = {
  ...
  articleDetail: (id: string) => [...KNOWLEDGE_BASE_KEYS.articles(), 'detail', id] as const,
  articleVersions: (id: string) => [...KNOWLEDGE_BASE_KEYS.articles(), 'versions', id] as const,
  articleComments: (id: string) => [...KNOWLEDGE_BASE_KEYS.articles(), 'comments', id] as const,
  articleAnalytics: (id: string) => [...KNOWLEDGE_BASE_KEYS.articles(), 'analytics', id] as const,
  categories: () => [...KNOWLEDGE_BASE_KEYS.all, 'categories'] as const,
  tags: () => [...KNOWLEDGE_BASE_KEYS.all, 'tags'] as const,
  search: (query: KnowledgeSearchRequest) => [...KNOWLEDGE_BASE_KEYS.all, 'search', query] as const,
...
export function useKnowledgeSearchQuery(request: KnowledgeSearchRequest, enabled = true) {
  return useQuery({
...
export function useRecentArticlesQuery(params?: { categoryId?: string; limit?: number }) {
  return useQuery({
...
export function useUpdateArticleMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (
      _data: unknown,
      variables: { id: string; data: Parameters<typeof KnowledgeBaseApi.updateArticle>[1] }
    ) => {
...
export function usePublishArticleMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (
      _data: unknown,
      variables: { id: string; request?: Parameters<typeof KnowledgeBaseApi.publishArticle>[1] }
    ) => {
...
export function useAddCommentMutation() {
  ...
  return useMutation({
    ...
    onSuccess: (
      _data: unknown,
      variables: { articleId: string; content: string; parentId?: string }
    ) => {
...
export function useBookmarkArticleMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (articleId: string) => KnowledgeBaseApi.bookmarkArticle(articleId),
    onSuccess: () => {
...
export function useSubmitForReviewMutation() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ articleId, reviewerId }: { articleId: string; reviewerId?: number }) =>
